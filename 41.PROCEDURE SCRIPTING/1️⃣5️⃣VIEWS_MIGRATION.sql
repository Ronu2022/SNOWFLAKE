...
CREATE SCHEMA IF NOT EXISTS DEV_EMP.PROC_SCHEMA;
...
CREATE OR REPLACE TABLE hrdata.employees
...
CREATE OR REPLACE  TABLE hrdata.departments
   ( dept_id NUMBER(4)
...
CREATE OR REPLACE  TABLE hrdata.locations
   ( location_id NUMBER(4)
...
CREATE OR REPLACE  TABLE hrdata.countries 
...
CREATE OR REPLACE  TABLE hrdata.regions
   ( region_id NUMBER NOT NULL 
   , region_name VARCHAR(25) 
   , PRIMARY KEY (region_id)
   );
   
CREATE OR REPLACE  TABLE hrdata.jobs
   ( job_id VARCHAR(10)
...
CREATE OR REPLACE  TABLE hrdata.job_history
...
;
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// RUN ABOVE CODES FOR THE TABLES:
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------



create or replace database proc_view_mig_db;
create or replace schema schema_proc_view_mig;
create or replace schema proc_view_mig_db.schema_procedures;

create or replace schema proc_view_mig_db.migrated_scema;

// CREATION OF VIEW:
create or replace view proc_view_mig_db.schema_proc_view_mig.emp_dept as 
select e.employee_id, concat(e.first_name,' ',e.last_name) as emp_name,e.dept_id, d.department_name
from DEV_EMP.hrdata.employees as e
join DEV_EMP.hrdata.departments as d on e.dept_id = d.dept_id;


select * from proc_view_mig_db.schema_proc_view_mig.emp_dept ; -- rfecfords

-- Now the requirement:
    -- we need to migrat the views
    -- llok direct migration would be same as that of table.
    -- but what if we have chanmged the placements of table making the view:
        -- DEV_EMP_2.hrdata.employees -> lets say that's migrated to a new locationh from dev env to a different env
        -- DEV_EMP_2. same --> lets say that's migrated to a new location from dev env to a diff env. 
// CREATE A DIFF ENV:


// table migration:

create or replace procedure proc_view_mig_db.migrated_scema.proc_miograte_tables(S_DB_PARAM varchar, S_SCH_PARAM varchar, T_DB_PARAM varchar, T_SCH_PARAM varchar, all_flag varchar, replace_flag varchar)
returns varchar
language sql
...
$$
    ...
    begin
        if(:all_flag = 'Y') then
            open CUR_ALL_TBL using(:S_DB_PARAM||'.INFORMATION_SCHEMA.TABLES', :S_SCH_PARAM);
                for rec in CUR_ALL_TBL do
                    SRCTBL := rec.TABLE_NAME;
                    select get_ddl('TABLE', :S_DB_PARAM||'.'||:S_SCH_PARAM||'.'||:SRCTBL) into :DDL_STATEMENT;
                    if(:replace_flag = 'Y') then
                        DDL_STATEMENT := REPLACE(:DDL_STATEMENT, 'create or replace TABLE ', 'create or replace TABLE '||:T_DB_PARAM||'.'||:T_SCH_PARAM||'.');
                    else
                        DDL_STATEMENT := REPLACE(:DDL_STATEMENT, 'create or replace TABLE ', 'create TABLE if not exists '||:T_DB_PARAM||'.'||:T_SCH_PARAM||'.');
                    end if; 
                    execute immediate :DDL_STATEMENT; 
                end for; 
            close CUR_ALL_TBL;
        else
            open CUR_SOME_TBL using(:S_DB_PARAM||'.'||'WORK'||'.'||'MIGRATE_TABLES');
                for rec_b in CUR_SOME_TBL do
                    SRCTBL := rec_b.TABLE_NAME;
                    select get_ddl('TABLE', :SRCTBL) into :DDL_STATEMENT;
                    if (:replace_flag = 'Y') then
                        DDL_STATEMENT := REPLACE(:DDL_STATEMENT, 'create or replace TABLE ', 'create or replace TABLE '||:T_DB_PARAM||'.'||:T_SCH_PARAM||'.');
                    else
                        DDL_STATEMENT := REPLACE(:DDL_STATEMENT, 'create or replace TABLE ', 'create TABLE if not exists '||:T_DB_PARAM||'.'||:T_SCH_PARAM||'.');
...
CALL proc_view_mig_db.migrated_scema.proc_miograte_tables('DEV_EMP', 'HRDATA','PROC_VIEW_MIG_DB', 'MIGRATED_SCEMA' ,'Y','Y'); -- migrated.
