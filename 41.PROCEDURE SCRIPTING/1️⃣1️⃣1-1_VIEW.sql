...
CREATE SCHEMA IF NOT EXISTS DEV_EMP.procs_schema;
...
select * from information_schema.columns where table_catalog = 'DEV_EMP' and table_schema = 'HRDATA';

select listagg(COLUMN_NAME,',') as column_name_agg from 
 information_schema.columns where table_catalog = 'DEV_EMP' and table_schema = 'HRDATA' and table_name = 'EMPLOYEES';

----------------------------------------------------------------------------------------------------------------------------

-- DEV_EMP.procs_schema;
create or replace procedure DEV_EMP.procs_schema.make_views(db_param varchar,schema_param varchar)
 returns varchar
 language sql 
 execute as caller 
 as
    declare
        cur CURSOR for 
                select distinct table_catalog as catl,
                                TABLE_SCHEMA as sch, 
                                TABLE_NAME  as tbl 
    ...
    begin
        open cur using(:db_param||'.information_schema.tables',:schema_param);
        ...
        for rec in cur do
            ...
            qs := 
                'CREATE OR REPLACE VIEW ' || db_name || '.' || schema_name || '.V_' || table_name || 
...
call DEV_EMP.procs_schema.make_views('DEV_EMP','HRDATA');
