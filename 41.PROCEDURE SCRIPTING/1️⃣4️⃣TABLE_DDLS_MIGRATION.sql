...
;
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT GET_DDL('TABLE','DEV_EMP.hrdata.employees');
/* O/P:
...
CREATE OR REPLACE PROCEDURE  DEV_EMP.PROC_SCHEMA.SP_MIGRATE_TABLE_DDLS(S_DB_PARAM VARCHAR,S_SCH_PARAM VARCHAR,T_DB_PARAM VARCHAR,T_SCH_PARAM VARCHAR,ALL_FLAG VARCHAR,REPLACE_FLAG VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL 
...
$$;
 ...
 -- LETS SAY MOVING FORM DEV ENV TO PROD ENVI
...
CREATE OR REPLACE SCHEMA STAGING_DB.STAGING_SCHEMA;

select * FROM STAGING_DB.INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE'; -- NO TABLES.

CALL DEV_EMP.PROC_SCHEMA.SP_MIGRATE_TABLE_DDLS('DEV_EMP','HRDATA','STAGING_DB','STAGING_SCHEMA','Y','Y');
